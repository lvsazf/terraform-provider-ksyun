...
func resourceKsyunRabbitmqInstance() *schema.Resource {
...
func resourceRabbitmqInstanceCreate(d *schema.ResourceData, meta interface{}) error {
	...
	createParam := []string{"availability_zone", "instance_id", "instance_name", "instance_password", "vpc_id",
		"subnet_id", "engine_version", "project_id", "project_Name", "bill_type", "duration", "mode",
		"instance_type", "ssd_disk", "node_num", "enablePlugins"}
	...
	if resp, err = conn.CreateInstance(&createReq); err != nil {
	...
	if createReq["AvailabilityZone"] != nil {
		az = createReq["AvailabilityZone"].(string)
	}
